...
CPMdist_plot <- function(DGE_object, categories){
  ...
  ggplot(long_format, aes(library, log10cpm)) + geom_violin(scale="count") + labs(x="Library", y="log10(CPM)")
...
MDS_plot <- function(DGE_object, categories){
  ...
  ggplot(mds_data, aes(dimension1, dimension2, color=categories)) +  geom_point() + geom_text_repel(size=3, aes(label=samples)) + labs(x="Leading LogFC dim1", y="Leading LogFC dim2", color="Category")
...
BCV_plot <- function(DGE_object, DGEEX_object){
  ...
  ggplot(BCV_result, aes(log2_aveCPM, tagWdis_bcv)) + geom_hline(yintercept=sqrt(DGE_object$common.dispersion), col="blue") + labs(x="log2(mean CPM)", y="Biological Coefficient of Variation") + geom_point(size=0.50)
...
MA_plot <- function(DGEEX_object, de_id, write_de){
  ...
  ggplot(MA_data, aes(log2_aveCPM, log2FC, color=deGenes)) + labs(x="log2(MeanCPM)", y="Log2(FC)") + geom_point(size=0.50) + geom_hline(yintercept=c(-1, 1), col="black") + scale_color_manual(values=c("red", "black", "blue")) + theme(legend.position="none")
...
Volcano_plot <- function(DGEEX_object, de_id){
  ...
  ggplot(Volc_data, aes(log2FC, log10_pval, color=deGenes)) + labs(x="Log2(FC)", y="-Log10(Pvalue)") + geom_point(size=0.50) + geom_vline(xintercept=c(-1, 1), col="black") + scale_color_manual(values=c("red", "black", "blue")) + theme(legend.position="none")
